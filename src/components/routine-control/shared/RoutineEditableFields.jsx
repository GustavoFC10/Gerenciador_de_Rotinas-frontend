...
import { formatDisplayDate } from '../details/routineDetailsUtils.js'
...
export function RoutineDueDateField({ task, onChange, appearance = 'card' }) {
  const [isEditing, setIsEditing] = useState(false)
...
function EditorShell({ label, appearance, children }) {
  const styles = fieldAppearance[appearance] ?? fieldAppearance.card
...
function ReadOnlyField({ label, value, icon, onEdit, appearance }) {
  const styles = fieldAppearance[appearance] ?? fieldAppearance.card
...
function finishEditingOnEscape(event, setIsEditing) {
  if (event.key === 'Escape') {
